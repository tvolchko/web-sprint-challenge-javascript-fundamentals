...
//🚀🚀🚀 ⬇️ 📝 Explanation ⬇️ 📝 🚀🚀🚀: 
// standard closure, a nested function can always look outward for variables, just not inward
...
const zooAnimals = [
  ...
  */
  ...
  function animalNames(arr){
    let displayNames = []
    arr.forEach((anim) => displayNames.push(`name: ${anim.animal_name}, scientific: ${anim.scientific_name}`))
    return displayNames
    // return arr.map((anim) => `name: ${anim.animal_name}, scientific: ${anim.scientific_name}`)
    // forEach uses three lines of code, while map uses one
}
  ...
  */
  ...
  function lowerCaseNames(arr){
    return arr.map((anim) => `${anim.animal_name.toLowerCase()}`)
  }
  ...
  */
  ...
  function lowPopulationAnimals(arr){
    return arr.filter((anim) => anim.population<5)
    /*Your Code Here*/
  }
  ...
  */
  ...
  function USApop(arr){
    return arr.reduce((num1, anim) => num1 + anim.population, 0)
    /*Your Code Here*/
  }
